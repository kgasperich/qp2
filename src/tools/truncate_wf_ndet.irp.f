...
subroutine routine_s2
 implicit none
 integer :: ndet_max, ndet_min
 double precision :: wmin
 integer(bit_kind), allocatable :: psi_det_tmp(:,:,:)
 double precision, allocatable :: psi_coef_tmp(:,:)
 double precision, allocatable :: weights_tmp(:)
 ...
 print*, 'Min ndet?'
 read(5,*) ndet_min
 ndet_min = min(ndet_min,N_det)
 print*, 'keeping at least ',ndet_min,' dets'

 allocate(weights_tmp(N_det),iorder(N_det))
 do i=1,N_det
   weights_tmp(i) = maxval(weight_configuration( det_to_configuration(i),:))
   iorder(i) = i
 enddo
 call dsort(weights_tmp, iorder, N_det)

 wmin = weights_tmp(N_det+1-ndet_min)
 deallocate(weights_tmp,iorder)
