...
def np2gz(fname,a):
    """
    save np array as ezfio gz

    Parameters
    ----------
    fname : str
        path to gz file where array will be saved
    a : numpy array
        array to be saved in ezfio gzip format
    """
    if np.issubdtype(a.dtype, np.integer):
        ...
        raise TypeError(f"Expected integer or real float, not {a.dtype}")

    with gzip.open(fname,'wb') as f:
        f.write((f'{len(a.shape):3d}\n').encode())
        f.write((' '.join(f'{i:20d}' for i in a.shape) + '\n').encode())
        for val in a.ravel(order='F'):
            f.write((f"{val:{fmt}}\n").encode())
    return
...
def gz2np(fname, dtype=float):
    ...
    ----------
    fname : str
        path to gz file to read array from
    dtype (optional) : type of data to read
    """
...
def gzobj2np2(f, dtype=float):
    """
    load ezfio array as np array
    takes gzip file object instead of file path
    iterates over values, so might have less overhead than gz2np

    Parameters
    ----------
    f : gzip file object
        gzip object to read array from
    dtype (optional) : type of data to read
    """
    #with gzip.open(fname) as f:
    ndim = int(f.readline().decode().strip())
...
def bytes2np(bstr, dtype=float):
    """
    load ezfio array as np array
    takes bytes-like object read from gzip file instead of file path

    Parameters
    ----------
    bstr : byte-like object
        bytes-like string to read array from
    dtype (optional) : type of data to read
    """
...
def bytes2np2(bstr, dtype=float):
    ...
    load ezfio array as np array
    takes bytes-like object read from gzip file instead of file path
    iterates over values, so might have less overhead than bytes2np
...
def str2np(s, dtype=float):
    """
    load ezfio array as np array
    takes string read from gzip file instead of file path

    Parameters
    ----------
    s : str
        string to read array from
    dtype (optional) : type of data to read
    """
    lines = s.strip().split('\n')
...
def save_mos_to_ezfio(mf,ezpath,save_symm=False):
    """
    input:
        mol, pyscf Mol object
        Mol object containing basis info to be written to ezfio
...
def save_pyscf_to_ezfio(mf, ezpath, save_symm = False):
    ...
    save_mos_to_ezfio(mf, ezpath, save_symm = save_symm)
    return
...
def save_1det_to_ezfio(mf, ezpath, hplist = None, n_det_max = 10000, selection_factor=0.2, save_symm=False):
